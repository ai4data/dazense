...
from dazense_core.commands.sync.cleanup import DatabaseSyncState, cleanup_stale_databases, cleanup_stale_paths
from dazense_core.config import AnyDatabaseConfig, DazenseConfig
from dazense_core.config.databases.base import DatabaseConfig
from dazense_core.templates.engine import get_template_engine
...
class DatabaseSyncProvider(SyncProvider):
    ...
    def pre_sync(self, config: DazenseConfig, output_path: Path) -> None:
        cleanup_stale_databases(config.databases, output_path, verbose=True)

    def get_items(self, config: DazenseConfig) -> list[AnyDatabaseConfig]:
