...
from dazense_core.ui import UI, ask_confirm
from dazense_core.version import clear_version_cache, get_latest_version, parse_version


def upgrade() -> None:
    """Upgrade dazense-core to the latest version."""
    ...
    # Perform upgrade
    UI.print(f"\n🔄 Upgrading dazense-core {current_version} → {latest_version}...\n")

    # Use uv if available, otherwise fallback to pip
    if shutil.which("uv"):
        cmd = ["uv", "pip", "install", "--upgrade", "dazense-core"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "dazense-core"]
    ...
    try:
        ...
        UI.print("  pip install --upgrade dazense-core")
