...
def setup_project_name(force: bool = False) -> tuple[str, Path, DazenseConfig | None]:
    """Setup the project name. Returns existing config if found and user wants to extend."""
    # Check if we're in a directory with an existing dazense_config.yaml
    current_dir = Path.cwd()
    config_file = current_dir / "dazense_config.yaml"

    if config_file.exists():
        # Load existing config to get project name
        existing_config = DazenseConfig.try_load(current_dir)
        if existing_config:
            UI.title("Found existing dazense_config.yaml")
...
def create_empty_structure(project_path: Path) -> tuple[list[str], list[CreatedFile]]:
    ...
    FILES = [
        ...
        CreatedFile(path=Path(".dazenseignore"), content="templates/\n*.j2\n"),
    ]
...
def init(
    *,
    ...
    Creates a project folder with a dazense_config.yaml configuration file.
    ...
    """
    UI.info("\n🚀 dazense project initialization\n")

    try:
        project_name, project_path, existing_config = setup_project_name(force=force)
        config = DazenseConfig.promptConfig(project_name, existing=existing_config)
        ...
        UI.success(f"Saved [dim]{project_path / 'dazense_config.yaml'}[/dim]")
        UI.print()
        UI.print("[bold green]Done![/bold green] Your dazense project is ready. 🎉")
...
[cyan]dazense debug[/cyan]   - Test connectivity to your configured databases and LLM
              Verifies that all connections are working properly

[cyan]dazense sync[/cyan]    - Sync database schemas to local markdown files
              Creates documentation for your tables and columns

[cyan]dazense chat[/cyan]    - Start the dazense chat interface
