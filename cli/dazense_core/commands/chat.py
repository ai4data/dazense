...
from dazense_core.mode import MODE
from dazense_core.tracking import track_command
from dazense_core.ui import create_console

console = create_console()

# Default port for the dazense chat server
SERVER_PORT = 5005
FASTAPI_PORT = 8005
SECRET_FILE_NAME = ".dazense-secret"


def get_server_binary_path() -> Path:
    """Get the path to the bundled dazense-chat-server binary."""
    # The binary is in the bin folder relative to this file
    cli_dir = Path(__file__).parent.parent
    bin_dir = cli_dir / "bin"
    binary_path = bin_dir / "dazense-chat-server"
    if sys.platform == "win32" and not binary_path.exists():
        binary_path = bin_dir / "dazense-chat-server.exe"
...
def chat():
    ...
    Launches the dazense chat server and opens the web interface in your browser.
    """
    console.print("\n[bold cyan]💬 Starting dazense chat...[/bold cyan]\n")

    # Try to load dazense config from current directory
    config = DazenseConfig.try_load(exit_on_error=True)
    assert config is not None  # Help type checker after exit_on_error=True
    console.print(f"[bold green]✓[/bold green] Loaded config from {Path.cwd() / 'dazense_config.yaml'}")
    ...
    try:
        ...
        if config and config.mcp:
            mcp_path = Path(config.mcp.json_file_path)
            if not mcp_path.is_absolute():
                mcp_path = (Path.cwd() / mcp_path).resolve()
            env["MCP_JSON_FILE_PATH"] = str(mcp_path)
            console.print("[bold green]✓[/bold green] Set MCP_JSON_FILE_PATH from config")

        env["DAZENSE_DEFAULT_PROJECT_PATH"] = str(Path.cwd())
