"""Context object for Jinja templates in the dazense context folder.

This module provides the `dazense` object that is exposed to user Jinja templates,
...
Example template usage:
    ...
    {{ dazense.notion.page('abc123').title }}
"""
...
if TYPE_CHECKING:
    from dazense_core.config.base import DazenseConfig
...
class NotionPage:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        if self._data is None:
            ...
            from dazense_core.commands.sync.providers.notion.provider import (
                extract_page_id,
                get_page_title,
                strip_images,
            )
...
class NotionProvider:
    ...
    def __init__(self, config: DazenseConfig):
        self._config = config
    ...
    def _get_api_key_for_page(self, page_url_or_id: str) -> str:
        ...
        """
        from dazense_core.commands.sync.providers.notion.provider import extract_page_id
    ...
    def page(self, page_url_or_id: str) -> NotionPage:
        ...
        Example:
            {{ dazense.notion.page('https://notion.so/My-Page-abc123').content }}
            {{ dazense.notion.page('abc123def456...').title }}
        """
...
class DazenseContext:
    """The main context object exposed as `dazense` in user templates.
    ...
    Example template usage:
        {{ dazense.notion.page('url').content }}
    ...
    @property
    def config(self) -> DazenseConfig:
        """Access the dazense configuration.
...
def create_dazense_context(config: DazenseConfig) -> DazenseContext:
    """Create a DazenseContext for template rendering.

    Returns:
        A DazenseContext instance to be used as `dazense` in templates.
    """
    return DazenseContext(config)
