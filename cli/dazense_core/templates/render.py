...
This module discovers and renders `.j2` files in the dazense context folder,
making the `dazense` object available for accessing provider data.
...
from .context import create_dazense_context
...
def discover_templates(
    ...
    Args:
        project_path: Path to the dazense project root.
...
def render_template(
    ...
    Args:
        ...
        project_path: Path to the dazense project root.
        config: The dazense configuration.
    ...
    # Create the dazense context
    dazense = create_dazense_context(config)

    # Load and render the template
    template = env.get_template(str(template_path))
    rendered = template.render(dazense=dazense)
