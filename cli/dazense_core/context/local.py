...
class LocalContextProvider(ContextProvider):
    ...
    def init(self) -> None:
        ...
        Raises:
            ...
            ValueError: If dazense_config.yaml is not found.
        """
        if not self.target_path.exists():
            raise FileNotFoundError(
                f"Context path does not exist: {self.target_path}\n"
                "For local mode, ensure the path is mounted as a Docker volume "
                "or use DAZENSE_CONTEXT_SOURCE=git for git-based context."
            )

        if not self.validate():
            raise ValueError(
                f"dazense_config.yaml not found in {self.target_path}\n"
                "Ensure the context path contains a valid dazense project."
            )
    ...
    def is_initialized(self) -> bool:
        ...
        Returns:
            True if target path exists and contains dazense_config.yaml.
        """
