...
from dazense_core.commands.chat import (
    chat,
    ensure_auth_secret,
...
from dazense_core.config.base import DazenseConfig, DazenseConfigError
...
def test_try_load_returns_none_when_file_not_found(tmp_path: Path):
    cfg = DazenseConfig.try_load(tmp_path)
    assert cfg is None
...
def test_try_load_returns_config_when_valid(tmp_path: Path):
    valid_yaml = tmp_path / "dazense_config.yaml"
    valid_yaml.write_text("project_name: test-project")

    cfg = DazenseConfig.try_load(tmp_path)

    assert cfg is not None
    assert isinstance(cfg, DazenseConfig)
...
def test_try_load_exits_on_file_not_found(tmp_path: Path):
    with patch("dazense_core.config.base.Console") as mock_console_cls:
        ...
        mock_console.print.assert_any_call("[bold red]✗[/bold red] No dazense_config.yaml found in current directory")
...
def test_try_load_exits_on_invalid_yaml(tmp_path: Path):
    ...
    with patch("dazense_core.config.base.Console") as mock_console_cls:
        ...
        assert "Failed to load dazense_config.yaml" in call_args
...
def test_try_load_raises_on_file_not_found(tmp_path: Path):
    with pytest.raises(DazenseConfigError) as exc_info:
        DazenseConfig.try_load(tmp_path, raise_on_error=True)

    assert "No dazense_config.yaml found" in str(exc_info.value)
...
def test_try_load_raises_on_validation_error(tmp_path: Path):
    ...
    assert "Failed to load dazense_config.yaml" in str(exc_info.value)
...
def test_get_server_binary_path():
    ...
    assert result_path.name == "dazense-chat-server"
...
def test_get_server_binary_path_does_not_exists():
    ...
    with patch("dazense_core.commands.chat.Path.exists", return_value=False):
        with pytest.raises(SystemExit) as exc_info:
            get_server_binary_path()
...
class TestEnsureAuthSecret:
    def test_returns_none_when_env_var_already_set(self, tmp_path: Path, monkeypatch):
        ...
        with patch("dazense_core.commands.chat.console"):
    ...
    def test_loads_existing_secret_from_file(self, tmp_path: Path):
        ...
        secret_file = tmp_path / ".dazense-secret"
...
class TestChatCommand:
    ...
    @patch("dazense_core.commands.chat.webbrowser.open")
    @patch("dazense_core.commands.chat.wait_for_server")
    @patch("dazense_core.commands.chat.subprocess.Popen")
    @patch("dazense_core.commands.chat.get_fastapi_main_path")
    @patch("dazense_core.commands.chat.get_server_binary_path")
    @patch("dazense_core.commands.chat.console")
    def test_chat_starts_both_servers(
        ...
        mock_binary_path.return_value = bin_dir / "dazense-chat-server"
    ...
    def test_chat_sets_environment_variables(
        ...
        assert "DAZENSE_DEFAULT_PROJECT_PATH" in env
...
""")
        ...
        bin_dir.mkdir()
        (bin_dir / "dazense-chat-server").touch()
