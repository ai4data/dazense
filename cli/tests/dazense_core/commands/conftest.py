...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("DAZENSE_DEFAULT_PROJECT_PATH", raising=False)


@pytest.fixture
def create_config(tmp_path, monkeypatch):
    """Factory fixture to create a dazense config file and chdir to tmp_path."""

    def _create(content: str = "project_name: test-project\n"):
        config_file = tmp_path / "dazense_config.yaml"
...
def mock_socket():
    ...
    with patch("socket.socket") as mock_socket_cls:
        ...
        with patch("dazense_core.commands.chat.sleep"):
            yield mock_sock
