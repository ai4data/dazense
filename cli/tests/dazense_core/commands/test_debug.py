...
from dazense_core.commands.debug import check_llm_connection, debug
from dazense_core.config.databases import BigQueryConfig, DuckDBConfig, PostgresConfig
from dazense_core.config.llm import LLMConfig, LLMProvider
...
class TestDebugCommand:
    ...
    def test_exits_when_no_config_found(self, tmp_path, monkeypatch):
        ...
        with patch("dazense_core.commands.debug.console"):
...
""")
        ...
        with patch(
            "dazense_core.config.databases.postgres.PostgresConfig.check_connection",
            return_value=(True, "Connected (5 tables found)"),
        ):
            with patch("dazense_core.commands.debug.console") as mock_console:
                debug()
...
""")
        ...
        with patch(
            "dazense_core.commands.debug.check_llm_connection",
...
""")
        ...
        with patch(
            "dazense_core.commands.debug.check_llm_connection", return_value=(False, "API key is not working")
        ) as mock_check:
            with patch("dazense_core.commands.debug.console") as mock_console:
                debug()
