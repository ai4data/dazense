...
To run locally with Docker:
    docker run -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=dazenseTesting123!" \\
        -p 1433:1433 mcr.microsoft.com/mssql/server:2022-latest

    MSSQL_HOST=localhost MSSQL_PASSWORD="dazenseTesting123!" \\
        MSSQL_DRIVER="/opt/homebrew/lib/libtdsodbc.so" \\
        uv run pytest tests/dazense_core/commands/sync/integration/test_mssql.py -v
"""
...
from dazense_core.config.databases.mssql import MssqlConfig
...
def temp_database():
    ...
    db_name = f"dazense_unit_tests_{uuid.uuid4().hex[:8].lower()}"
