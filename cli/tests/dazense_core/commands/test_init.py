...
from dazense_core.commands.init import (
    CreatedFile,
...
from dazense_core.config.exceptions import InitError
...
class TestCreateEmptyStructure:
    ...
    def test_creates_dazenseignore_file(self, tmp_path: Path):
        """Creates .dazenseignore file with templates/ entry."""
        folders, files = create_empty_structure(tmp_path)

        dazenseignore_file = tmp_path / ".dazenseignore"
        assert dazenseignore_file.exists()
        content = dazenseignore_file.read_text()
    ...
    def test_returns_created_files_list(self, tmp_path: Path):
        ...
        assert Path(".dazenseignore") in file_paths
...
class TestSetupProjectName:
    ...
    @patch("dazense_core.commands.init.ask_text")
    ...
    @patch("dazense_core.commands.init.ask_confirm")
    @patch("dazense_core.commands.init.DazenseConfig.try_load")
    def test_reinitializes_existing_project(self, mock_try_load, mock_confirm, tmp_path: Path, monkeypatch):
        ...
        (tmp_path / "dazense_config.yaml").write_text("project_name: existing\n")
...
class TestDazenseConfigPromptDatabases:
    """Tests for DazenseConfig._prompt_databases method."""

    @patch("dazense_core.config.base.ask_confirm")
    def test_returns_empty_list_when_user_skips(self, mock_confirm):
        ...
        result = DazenseConfig._prompt_databases()

        assert result == []

    @patch("dazense_core.config.base.ask_confirm")
    @patch("dazense_core.config.base.ask_select")
    @patch("dazense_core.config.databases.duckdb.DuckDBConfig.promptConfig")
...
class TestDazenseConfigPromptRepos:
    """Tests for DazenseConfig._prompt_repos method."""
    ...
    def test_returns_empty_list_when_user_skips(self, mock_confirm):
        ...
        result = DazenseConfig._prompt_repos()

        assert result == []

    @patch("dazense_core.config.base.ask_confirm")
    @patch("dazense_core.config.repos.base.RepoConfig.promptConfig")
    def test_adds_repository(self, mock_prompt_config, mock_confirm):
        """Adds repository when configured."""
        from dazense_core.config import DazenseConfig
        from dazense_core.config.repos import RepoConfig
...
class TestDazenseConfigPromptLLM:
    """Tests for DazenseConfig._prompt_llm method."""
    ...
    def test_returns_none_when_user_skips(self, mock_confirm):
        ...
        result = DazenseConfig._prompt_llm()

        assert result is None

    @patch("dazense_core.config.base.ask_confirm")
    @patch("dazense_core.config.llm.LLMConfig.promptConfig")
    def test_creates_llm_config(self, mock_prompt_config, mock_confirm):
        """Creates LLM config when configured."""
        from dazense_core.config import DazenseConfig, LLMConfig, LLMProvider
    ...
    @patch("dazense_core.config.llm.ask_text")
    @patch("dazense_core.config.llm.ask_select")
    def test_raises_on_empty_api_key(self, mock_select, mock_text):
        """Raises error when API key is empty (handled by required_field)."""
        from dazense_core.config import LLMConfig
...
class TestDazenseConfigPromptSlack:
    """Tests for DazenseConfig._prompt_slack method."""
    ...
    def test_returns_none_when_user_skips(self, mock_confirm):
        ...
        result = DazenseConfig._prompt_slack()

        assert result is None

    @patch("dazense_core.config.base.ask_confirm")
    @patch("dazense_core.config.slack.SlackConfig.promptConfig")
    def test_creates_slack_config(self, mock_prompt_config, mock_confirm):
        """Creates Slack config when configured."""
        from dazense_core.config import DazenseConfig, SlackConfig
...
class TestInitCommand:
    ...
    @patch("dazense_core.commands.init.DazenseConfig.promptConfig")
    @patch("dazense_core.commands.init.setup_project_name")
    @patch("dazense_core.commands.init.UI")
    def test_init_creates_config_file(
        ...
        """Init command creates dazense_config.yaml file."""
        ...
        mock_prompt_config.return_value = DazenseConfig(
        ...
        config_file = project_path / "dazense_config.yaml"
    ...
    def test_init_shows_updated_message_for_existing_config(
        ...
        existing_config = DazenseConfig(project_name="existing-project")
    ...
    @patch("dazense_core.commands.debug.debug")
    @patch("dazense_core.commands.init.DazenseConfig.promptConfig")
    @patch("dazense_core.commands.init.setup_project_name")
    @patch("dazense_core.commands.init.UI")
    def test_init_runs_debug_when_config_has_databases(
        ...
        from dazense_core.commands.init import init
        from dazense_core.config import DazenseConfig
        from dazense_core.config.databases.duckdb import DuckDBConfig
